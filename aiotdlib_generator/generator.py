...
class Generator:
        ...
        with open(f"{self.destination}/types/__init__.py", "w", encoding="utf-8") as f:
            f.write(
                section_init_file_template.render(
                    notice=self.notice, entities=[e for e in entities if e.is_constructor]
                )
            )
    ...
    def render_functions_init_file(self, entities: Entities):
        ...
        with open(f"{self.destination}/functions/__init__.py", "w", encoding="utf-8") as f:
            f.write(
                section_init_file_template.render(notice=self.notice, entities=[e for e in entities if e.is_function])
            )
    ...
    def generate(self):
        ...
        os.system(f"ruff format -s {self.destination}")
